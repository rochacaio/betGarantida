...
type Leg = { id: string; bookmakerId: string; stake: number; odd: number; commission: number; cashback: number; increase: number; result: "PENDING" | "WON" | "LOST" };
type Surebet = { id: string; title: string; event: string; date: string; status: "OPEN" | "SETTLED"; profit: number; roi: number; legs: Leg[] };
...
const initialSurebets: Surebet[] = [
  ...
  {
    id: "op-1038", title: "Arbitragem #1038", event: "Flamengo x Grêmio", date: "08 ago, 21:00", status: "SETTLED", profit: 28.4, roi: 4.18,
    legs: [
...
function Surebets({ surebets, bookmakers, navigate, edit }: { surebets: Surebet[]; bookmakers: Bookmaker[]; navigate: (s: Screen) => void; edit: (s: Surebet) => void }) {
  return <><Topbar title="Minhas entradas" subtitle="Consulte, acompanhe e liquide suas operações" action={<button className="primary" onClick={() => navigate("editor")}>+ Nova surebet</button>} /><section className="content"><div className="tabs"><button className="active">Todas <span>{surebets.length}</span></button><button>Em aberto <span>{surebets.filter(s => s.status === "OPEN").length}</span></button><button>Liquidadas <span>{surebets.filter(s => s.status === "SETTLED").length}</span></button></div><article className="panel list-panel"><div className="filters"><div className="search grow">⌕ <input placeholder="Buscar por evento ou casa..." /></div><button className="secondary">Agosto 2026⌄</button><button className="secondary">Filtros</button></div><SurebetTable surebets={surebets} bookmakers={bookmakers} onEdit={edit} /></article></section></>;
}

function LegRow({ leg, index, bookmakers, update, remove }: { leg: Leg; index: number; bookmakers: Bookmaker[]; update: (patch: Partial<Leg>) => void; remove: () => void }) {
  const b = bookmakers.find(x => x.id === leg.bookmakerId);
  const effectiveOdd = 1 + (leg.odd - 1) * (1 + leg.increase / 100) * (1 - leg.commission / 100);
  return <div className="leg-row"><div className="leg-number">{String.fromCharCode(65 + index)}</div><Field label="Casa de aposta"><select value={leg.bookmakerId} onChange={e => update({ bookmakerId: e.target.value })}>{bookmakers.map(x => <option key={x.id} value={x.id}>{x.name} · {money.format(x.balance)}</option>)}</select></Field><Field label="Valor"><div className="money-input compact"><span>R$</span><input type="number" min="0" step="0.01" value={leg.stake} onChange={e => update({ stake: Number(e.target.value) })} /></div></Field><Field label="ODD"><input type="number" min="1.01" step="0.01" value={leg.odd} onChange={e => update({ odd: Number(e.target.value) })} /></Field><Field label="Comissão"><div className="percent-input"><input type="number" min="0" max="99" value={leg.commission} onChange={e => update({ commission: Number(e.target.value) })} /><span>%</span></div></Field><Field label="Cashback"><div className="percent-input"><input type="number" min="0" value={leg.cashback} onChange={e => update({ cashback: Number(e.target.value) })} /><span>%</span></div></Field><Field label="Aumento"><div className="percent-input"><input type="number" min="0" value={leg.increase} onChange={e => update({ increase: Number(e.target.value) })} /><span>%</span></div></Field><div className="effective"><span>ODD efetiva</span><strong>{effectiveOdd.toFixed(3)}</strong><small>{b?.name}</small></div>{index > 1 && <button className="remove-leg" onClick={remove}>−</button>}</div>;
}

function Editor({ bookmakers, editing, onSave, cancel }: { bookmakers: Bookmaker[]; editing?: Surebet; onSave: (s: Surebet) => void; cancel: () => void }) {
  const [event, setEvent] = useState(editing?.event ?? "Palmeiras x Corinthians");
  const [legs, setLegs] = useState<Leg[]>(editing?.legs ?? [
    { id: uid(), bookmakerId: bookmakers[0]?.id ?? "", stake: 100, odd: 3, commission: 0, cashback: 0, increase: 0, result: "PENDING" },
    { id: uid(), bookmakerId: bookmakers[1]?.id ?? bookmakers[0]?.id ?? "", stake: 115.38, odd: 2.6, commission: 0, cashback: 0, increase: 0, result: "PENDING" },
    { id: uid(), bookmakerId: bookmakers[2]?.id ?? bookmakers[0]?.id ?? "", stake: 76.92, odd: 3.9, commission: 0, cashback: 0, increase: 0, result: "PENDING" },
  ]);
  ...
  const save = () => onSave({ id: editing?.id ?? `op-${Date.now().toString().slice(-4)}`, title: editing?.title ?? `Arbitragem #${Date.now().toString().slice(-4)}`, event, date: editing?.date ?? "12 ago, agora", status: editing?.status ?? "OPEN", profit, roi, legs });
  return <><Topbar title={editing ? "Editar surebet" : "Nova surebet"} subtitle="Distribua as entradas e confira o retorno antes de salvar" action={<div className="header-actions"><button className="secondary" onClick={cancel}>Cancelar</button><button className="primary" onClick={save}>Salvar entrada</button></div>} /><section className="content editor-content"><div className="editor-intro"><div className="field event-field"><span>EVENTO OU IDENTIFICAÇÃO</span><input value={event} onChange={e => setEvent(e.target.value)} placeholder="Ex.: Palmeiras x Corinthians" /></div><div className="operation-summary"><div><span>Investimento</span><strong>{money.format(total)}</strong></div><div><span>Lucro estimado</span><strong className="positive-text">+ {money.format(profit)}</strong></div><div><span>ROI</span><strong className="positive-text">{pct(roi)}</strong></div></div></div>
      <article className="surebet-builder"><div className="builder-top"><div><span className="card-label">DISTRIBUIÇÃO DA SUREBET</span><h2>Entradas da operação</h2></div><div className="guarantee"><span>✓</span><div><small>Retorno garantido</small><strong>{money.format(Math.min(...returns))} | {pct(roi)}</strong></div></div></div><div className="legs">{legs.map((leg, index) => <LegRow key={leg.id} leg={leg} index={index} bookmakers={bookmakers} update={patch => setLegs(legs.map(x => x.id === leg.id ? { ...x, ...patch } : x))} remove={() => setLegs(legs.filter(x => x.id !== leg.id))} />)}</div><button className="add-leg" onClick={() => setLegs([...legs, { id: uid(), bookmakerId: bookmakers[0]?.id ?? "", stake: 0, odd: 2, commission: 0, cashback: 0, increase: 0, result: "PENDING" }])}>＋ Adicionar outra entrada</button></article>
...
export default function Home() {
  ...
  const [editing, setEditing] = useState<Surebet | undefined>();
  ...
  const navigate = (s: Screen) => { if (s === "editor") setEditing(undefined); setScreen(s); };
  const save = (surebet: Surebet) => { setSurebets(prev => prev.some(s => s.id === surebet.id) ? prev.map(s => s.id === surebet.id ? surebet : s) : [surebet, ...prev]); setScreen("surebets"); };
  const body = useMemo(() => {
    ...
    if (screen === "surebets") return <Surebets surebets={surebets} bookmakers={bookmakers} navigate={navigate} edit={s => { setEditing(s); setScreen("editor"); }} />;
    return <Editor bookmakers={bookmakers} editing={editing} onSave={save} cancel={() => setScreen("surebets")} />;
  }, [screen, surebets, bookmakers, editing]);
